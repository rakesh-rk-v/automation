...
def test_customer_requests(driver, screenshot_manager, _data_index):
    ...
    try:
        ...
        # Step: Checking Whether the Billing Mode Got Updated or Not
        with allure.step("Checking Whether the Billing Mode Got Updated or Not"):
            after_change_mode = db.get_bill_deleivery_mode(customer_id)
            if after_change_mode == previous_mode:
                logging.error("Bill Mode Did Not Changed")
                assert  "Bill Delivery Mode is Not Changed."
            else:
                logging.info("Bill Delivery Mode is Updated ")

            # Mapping values to delivery types
            delivery_type_after = "Soft Copy" if after_change_mode == 1 else "Hard/Physical Copy"
            delivery_type_previous = "Soft Copy" if previous_mode == 1 else "Hard/Physical Copy"

            logging.info(f"Previous Delivery Mode: {delivery_type_previous}")
            logging.info(f"Updated Delivery Mode: {delivery_type_after}")
