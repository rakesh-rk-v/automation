...
class CmsDataUtil(MySqlUtil):
    ...
    def get_work_order_id(self, cust_id):
    ...
    def get_activity_code_from_activities(self, activity_name):
        query = """SELECT * FROM activities WHERE tenant_id={}
                   AND activity_name='{}'
    ...
    def get_customer_contract_status_for_po_to_hy(self, cust_id):
        query = """SELECT * FROM customer_contracts 
        WHERE customer_id={} 
        AND tenant_id={}""".format(cust_id, self._tenant_id)
        result = self.execute_select_query(query)
        return result

    def get_customer_contract_hit(self, contract_id, cust_id):
        query = """SELECT * FROM customer_contracts_hist 
        WHERE tenant_id={} 
        AND customer_id={} 
        AND customer_contract_id={}
        ORDER BY hist_record_id DESC
        """.format(self._tenant_id, cust_id, contract_id)
        result = self.execute_select_query(query)
        return result
    ...
    def get_msisdn_from_contract_device(self, cust_id, cust_acc_id):
        query = """SELECT * FROM contract_devices 
        WHERE tenant_id={} 
        AND customer_id={}
        AND customer_account_id={}""".format(self._tenant_id, cust_id, cust_acc_id)
        result = self.execute_select_query(query)
        return result
    ...
    def update_work_order_table(self, cust_id, wo_id):
        logging.info(
            "Executing the Update Query For Work Order for Customer_ID ={} and Work Order ID = {} ".format(cust_id,
                                                                                                           wo_id))
    ...
    def get_cust_acc_status_by_custid(self, customer_id: int):
        query = """ select CUSTOMER_ACCOUNT_ID , CUSTOMER_ID ,BILLING_ACCOUNT_ID ,CUSTOMER_ACCOUNT_NAME ,STATUS  from dxpoms.customer_accounts ca where CUSTOMER_ID ={}""".format(
        customer_id)
        results = self.execute_select_query(query)
        return results
    ...
    def check_bill_cycle_exists_in_db_or_not(self, bill_cycle_id):
        query = "select * from dxpbill.bill_cycles bc where TENANT_ID ={} and BILL_CYCLE_ID = '{}'".format(self._tenant_id,
                                                                                                           bill_cycle_id)
        result = self.execute_select_query(query)
